...
def segment_TGSE(file_path):
    ...
    _, contours2, _ = cv2.findContours(
        markers, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
if __name__ == '__main__':
    ...
    loop = asyncio_test.get_event_loop()
    tasks = []
    ...
    loop.run_until_complete(asyncio_test.wait(tasks))
    loop.close()
